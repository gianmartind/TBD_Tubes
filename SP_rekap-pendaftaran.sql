...
AS
	...
	DECLARE
		...
		@queryFROM varchar(MAX) = 'FROM TB_Pendaftaran INNER JOIN TB_Penduduk ON TB_Pendaftaran.fk_nik = TB_Penduduk.nik INNER JOIN TB_Faskes ON TB_Pendaftaran.fk_idFaskes = TB_Faskes.idFaskes INNER JOIN TB_AlamatFaskes ON TB_Faskes.fk_idAlamatFaskes = TB_AlamatFaskes.idAlamatFaskes INNER JOIN TB_Kota ON TB_AlamatFaskes.fk_idKota = TB_Kota.idKota INNER JOIN TB_Provinsi ON TB_Kota.fk_idProvinsi = TB_Provinsi.idProvinsi ',
		--filter tanggal
